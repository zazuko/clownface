...
class Dataset {
  ...
  in (predicates) {
    if (predicates) {
      predicates = toArray(predicates).map(predicate => term(predicate))
    }
  ...
  has (predicates, objects) {
    ...
    if (objects) {
      objects = toArray(objects).map(object => term(object))
    }
  ...
  addIn (predicates, subjects, callback) {
    ...
    predicates = toArray(predicates).map(predicate => term(predicate))
    subjects = toArray(subjects).map(subject => term(subject))
  ...
  addList (predicates, items) {
    ...
    predicates = toArray(predicates).map(predicate => term(predicate))
    items = toArray(items).map(item => term(item))
